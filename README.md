[![tag][a]][1]
[![release][b]][2]
[![download][c]][3]
# Table of Contents <a name="anchor_main"></a>
---
1. [Praat\_script\_basics](#anchor_1) <br></br>
2. [References](#anchor_ref) <br></br>

## Praat\_script\_basics <a name="anchor_1"></a> [up](#anchor_main)
1. Information Window -> Printing
2. Variables -> Numeric & String
3. Data Structures -> Array & Dictionary
4. Flow Control -> For & If & While
5. Procedure -> Function-ish
6. Objects
7. User Interaction -> Form == GUI

## References <a name="anchor_ref"></a> [up](#anchor_main)
Please check Git repository for [latest update][4]

Please send any question to: <kwb425@icloud.com>

<!--Links to addresses, reference Markdowns-->
[1]: https://github.com/kwb425/Pratt_Script_Introduction_Pratt/tags
[2]: https://github.com/kwb425/Pratt_Script_Introduction_Pratt/releases
[3]: https://github.com/kwb425/Pratt_Script_Introduction_Pratt/releases
[4]: https://github.com/kwb425/Pratt_Script_Introduction_Pratt.git
<!--Links to images, reference Markdowns-->
[a]: https://img.shields.io/badge/Tag-v1.3-red.svg?style=plastic
[b]: https://img.shields.io/badge/Release-v1.3-green.svg?style=plastic
[c]: https://img.shields.io/badge/Download-Click-blue.svg?style=plastic